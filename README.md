...
`LanguageServer.jl` must be installed using julia's package manager.
Once installed, the vim plugin will start the Language Server if julia is in the path.
Add the following to your `init.vim` in a `lua` block:
